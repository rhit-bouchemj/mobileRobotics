...
int moves[16] = {};
int occGrid[6][6] = {};
int startX;	//changes to current X per loop
int startY;	//changes to current Y per loop
int goalX;	//Never changes
int goalY;	//Never changes
...
void navMaze(int startX, int startY, int goalX, int goalY, char *moveList, int currentMoveNumber) {
  diffX = goalX - startX;
  diffY = goalY - startY;
  ...
  //Move in desired X
  if(diffX < 0) {	//want to move left?
    if(occGrid[startY][startX - 1] == 0 && previousMove != 'R') {		// check desired spot is unoccupied
        moveSpace(-1, 0, currentMoveNumber, &moveList); // Move robot to desired Direction <-- update position + save move to array
        navMaze(startX-1, startY, goalX, goalY, &moveList, currentMoveNumber+1);
        // popOff(); //<-- pop last direction of order off of the list (will this pop all off the list if it reaches end)
    }
    else if(diffX > 0) {   //want to move Right //TODO: vertical was occupied -> Need to add here?
      if(occGrid[startY][startX + 1] == 0 && previousMove != 'L') {		// check desired spot is unoccupied
        moveSpace(1, 0, currentMoveNumber, &moveList); // Move robot to desired Direction <-- update position + save move to array
        navMaze(startX+1, startY, goalX, goalY, &moveList, currentMoveNumber+1);
    }
  }
  ...
  //Move in desired Y
  if(diffY < 0) { //want to move Up?
    if(occGrid[startY-1][startX] == 0 && previousMove != 'D') { 
      moveSpace(0, -1, currentMoveNumber, &moveList);
      navMaze(startX, startY-1, goalX, goalY, &moveList, currentMoveNumber+1);
    }
  }
  else if (diffY > 0) {
    if(occGrid[startY+1][startX] == 0 && previousMove != 'U') { 
      moveSpace(0, 1, currentMoveNumber, &moveList);
      navMaze(startX, startY+1, goalX, goalY, &moveList, currentMoveNumber+1);
    }
  }
  ...
  //Move pseudorandomly
  //Try to Move right
  if(occGrid[startY][startX + 1] == 0 && previousMove != 'L') //space must be free AND didn't move exact opposite last turn
  {
    moveSpace(1, 0, currentMoveNumber, &moveList);
    navMaze(startX+1, startY, goalX, goalY, &moveList, currentMoveNumber+1);
  }
  //Try to Move left
  if(occGrid[startY][startX - 1] == 0 && previousMove != 'R')
  {
    moveSpace(-1, 0, currentMoveNumber, &moveList);
    navMaze(startX-1, startY, goalX, goalY, &moveList, currentMoveNumber+1);
  }
  ...
  moveSpace(0,0,currentMoveNumber, &moveList);
  return;
}
